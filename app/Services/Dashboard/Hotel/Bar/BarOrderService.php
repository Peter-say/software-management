...
class BarOrderService
{
    public function validatedData(array $data)
    {
        $validator = Validator::make($data, [
            ...
            'items.*.id' => 'required|exists:bar_items,id',
    ...
    public function saveOrder(Request $request)
    {
      ...
      $validatedData = $this->validatedData($request->all()); // Validate the data
        ...
        $barOrder = BarOrder::create($data);
        if ($barOrder) {
            $barOrder->update(['status' => StatusConstants::OPENED]);
        }
        ...
        $barOrder->total_amount = $totalAmount + $data['tax_amount'] - $data['discount_amount'];
        $barOrder->save();

        // Notify bar staff
        $user = Auth::user();
        $roleService = new HotelServiceRole();
        $barStaff = HotelUser::whereIn('role', $roleService->userCanAccessSalesRole())->get();
        foreach ($barStaff as $staff) {
            if ($staff->user && $staff->user->email) {
                $staff->user->notify(new BarOrderNotification($barOrder, $staff->user, $roleService));
            }
        }
